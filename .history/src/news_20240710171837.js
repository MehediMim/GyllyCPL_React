...
const News = () => {
    ...
    return (
        <div className="bg-slate-200 pt-20 min-h-screen">
            ...
            <div className=" flex flex-wrap flex-col md:flex-row items-center justify-items-start mx-2 md:mx-20">
                {
                    ALLNews.map((news, index) => (
                        <div className="w-full md:w-1/4 h-72 p-1 " onClick={() => expandNews(news)}>
                            <div className="bg-white">
                                <div className="h-40 overflow-hidden"><img src={news[0]} alt="news" className=" w-full md:w-80 h-48 object-cover"></img></div>
                                <div className="flex justify-center items-center px-1 w-full h-20  md:text-sm xl:text-xl relative   text-slate-800 font-Titillium overflow-hidden">{news[1]}</div>
                            </div>
                        </div>
                    ))
                }
