...
const runDivs=[
    ["RIPON","SHOT GUN",61,NM69,"#C97925"],
    ["IMRAN","SYNDICATE",52,Syndicate,"#266B63"],
    ["MISHU","SYNDICATE",50,Syndicate,"#266B63"],
    ["JN SAKIB","SHOTGUN",43,ShotGun,"#EFBF46"],
    ["PARTHO","MACHINEGUN",42,MachineGun,"#5687A8"],
...
const ChngingDiv=()=>{
    ...
    useEffect(()=>{
        const interval = setInterval(()=>{
            setvarDiv((prevVarDiv) => (prevVarDiv + 1) % 3);
        },1500);
...
const ChngingRunDiv=()=>{
    ...
    return (
        <div className=' w-full h-full  overflow-hidden relative' style={{backgroundColor:runDivs[varDiv][3]}}>
            <div className='absolute  text-white opacity-15  h-full w-full ml-44  flex justify-center items-center'>
                <div className='text-9xl font-body'>{divs[varDiv][1]}</div>
            </div>
            <div className='absolute bottom-20 w-full flex justify-center font-Titillium text-white text-xl'>MOST RUNS</div>
            <div className='absolute bottom-10 w-full flex justify-center font-Saira text-white text-3xl'>{runDivs[varDiv][0]}</div>
            <div className='absolute bottom-20 w-full flex justify-center font-body text-white text-7xl'>{divs[varDiv][1]}</div>
            <div className='absolute flex justify-center items-center h-56  pl-10'><img src={divs[varDiv][2]} className='w-20 ' alt='LL'></img></div>
            <div className='absolute  w-80  opacity-20'><img src={divs[varDiv][2]} alt='LL'></img></div>
        </div>
